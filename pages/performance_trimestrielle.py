...
def performance_trimestrielle_page():
    ...
    if role == 'parent':
        ...
        if eleve_info:
            ...
            notes_eleve = get_notes_eleve_trimestrielles(matricule_eleve=matricule_eleve) 
            if notes_eleve:
                ...
                for matiere, note in notes_eleve.items():
                    ...
                    if st.session_state['trimestre'] > 1:
                        notes_precedentes = get_notes_eleve_precedentes(matricule_eleve=matricule_eleve, matiere=matiere, trimestre_precedent=st.session_state['trimestre'] - 1)
        ...
        if eleves:
            ...
            if eleve_selectionne:  
                ...
                # Afficher les notes de l'élève
                notes_eleve = get_notes_eleve_trimestrielles(eleve_name=eleve_selectionne) 
                if notes_eleve:
                    matieres = list(notes_eleve.keys())
                    notes = list(notes_eleve.values())
...
def get_matieres_for_class(class_name):
    ...
    cursor = db.cursor()
    sql = """
        SELECT me.matiere_enseignee, e.nom
        FROM matieres_des_enseignants me
        JOIN enseignants_classes ec ON me.identifiant_enseignant = ec.identifiant_enseignant
        JOIN enseignants e ON me.identifiant_enseignant = e.identifiant
        WHERE ec.classe = %s
    """
    cursor.execute(sql, (class_name,))
    ...
    return [{"matiere": row[0], "nom": row[1]} for row in matieres]
        ...
        for matiere in get_matieres_for_class(class_name):
            ...
            if note:
                ...
                if matricule_eleve:
                    ...
                    sql = "INSERT INTO notes_trimestrielles (note_id, trimestre) VALUES (%s, %s)"
...
def get_notes_eleve_trimestrielles(eleve_name=None, matricule_eleve=None):
    db = get_db_connection()
    cursor = db.cursor()
...
def get_notes_eleve_precedentes(eleve_name=None, matricule_eleve=None, matiere=None, trimestre_precedent=None):
    db = get_db_connection()
    cursor = db.cursor()
    ...
    if matricule_eleve is not None:
        ...
        sql = """
            ...
            JOIN eleves e ON n.matricule_eleve = e.matricule_eleve
            JOIN notes_trimestrielles ns ON n.id = ns.note_id
            WHERE n.matricule_eleve = %s AND n.matiere = %s AND ns.trimestre = %s
        """
        cursor.execute(sql, (matricule_eleve, matiere, trimestre_precedent))
    else:
        # Utiliser le nom de l'élève (pour l'admin et l'admin principal)
        sql = """
            SELECT n.note
            ...
            JOIN eleves e ON n.matricule_eleve = e.matricule_eleve
            JOIN notes_trimestrielles ns ON n.id = ns.note_id
            WHERE CONCAT(e.nom, ' ', e.prenom) = %s AND n.matiere = %s AND ns.trimestre = %s
        """
...
if __name__ == "__main__":
    performance_trimestrielle_page()